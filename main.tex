...
\section{Návrh a implementace grafického editoru ER diagramů}
	Grafický editor představuje jádro celého CASE nástroje, protože většina operací, které nástroj umožňuje, je prováděna prostřednictvím jeho rozhraní. Nástroj je implementován jako desktopová aplikace určena pro operační systém Microsoft Windows, konkrétně se jedná o WPF aplikaci, pro její implementaci byl použit programovací jazyk C\#. 
	
	Windows Presentation Foundation je použito z důvodu snadného oddělení prezentační a aplikační vrstvy při zachování jednoduchosti událostmi řízeného programování jako je tomu u WinForms aplikací. 
